...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '918921483992@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./gif/mention.mp4'), MessageType.gif, { mimetype: Mimetype.mp4, quoted : message.data, ptt: false})
    }
...
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
   await message.client.sendMessage(message.jid, fs.readFileSync('./gif/' + a + '.mp3'), MessageType.gif, { mimetype: Mimetype.mp4, quoted: message.data, ptt: false})
}
});
}
